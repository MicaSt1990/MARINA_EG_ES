# -*- coding: utf-8 -*-
"""
Created on Thu Mar 19 22:31:06 2026
...
# Cambiado a la ruta del SPI
SPI_FILE = BASE_DIR / "SPEI_SPI_recortado" / "SPI_03_Egipto_1981_2024.nc"
...
# =============================================================================
print("Cargando datos SPI-03 y máscara...")
ds = nc.Dataset(SPI_FILE)
try:
    # Intenta leer spi03, si falla busca spi
    spi_raw = ds.variables["spi03"][:]
except KeyError:
    spi_raw = ds.variables["spi"][:]
ds.close()

spi_3d = np.ma.filled(spi_raw.astype(np.float64), np.nan)
spi_3d[(spi_3d < -10) | (spi_3d > 10)] = np.nan
...
# =============================================================================
print("Calculando Área y Ranking estacional para 3 umbrales (SPI)...")
fechas = []
area_mod, rank_mod = [], []
area_ext, rank_ext = [], []
area_vex, rank_vex = [], []
...
for year in range(año_inicio, año_fin + 1):
    for m in meses_idx:
        ...
        # Usamos spi_3d en lugar de spei_3d
        mapa_tierra = np.where(np.isnan(mask), np.nan, spi_3d[t, :, :])
...
# Etiquetas de la leyenda cambiadas a SPI
ax1.plot(fechas, rank_mod, color=c_mod, linestyle=ls_mod, linewidth=lw_mod, label=f'SPI ≤ {THR_MOD}')
ax1.plot(fechas, rank_ext, color=c_ext, linestyle=ls_ext, linewidth=lw_ext, label=f'SPI ≤ {THR_EXT}')
ax1.plot(fechas, rank_vex, color=c_vex, linestyle=ls_vex, linewidth=lw_vex, label=f'SPI ≤ {THR_VEX}')
...
# Guardar con nombre SPI03
plt.savefig(OUT_DIR / "Seasonal_Drought_Ranking_SPI03_Final.png", dpi=300, bbox_inches='tight')
plt.savefig(OUT_DIR / "Seasonal_Drought_Ranking_SPI03_Final.pdf", dpi=300, bbox_inches='tight')
